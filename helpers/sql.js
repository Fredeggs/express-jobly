...
function sqlForFiltering(filters, jsToSql) {
  console.log("These are the filters passed in: ", filters);
  const keys = Object.keys(filters);
  if (keys.length === 0) throw new BadRequestError("No data");
  if (filters.minEmployees && filters.maxEmployees){
    if(filters.minEmployees > filters.maxEmployees) throw new ExpressError("Minimum number of employees cannot be greater than maximum number of employees")
  }
  ...
  return {
    setFilters: `WHERE ${cols.join(" AND ")}`,
